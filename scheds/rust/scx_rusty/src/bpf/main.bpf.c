...
struct pcpu_ctx {
	...
	u64 task_vtime; /* vtime of the task (minus slice_ns) currently on the cpu*/
		   ...
		   u64 wake_flags)
{
	 ...
	 * preempted i.e. if the task p's vtime is less the vtime (minus a small
	 * buffer: slice_ns) of any other currently runnign task. If there is, then
	 * set preempt that cpu. This is to increase responsiveness for higher
	 * priority tasks. The task might be forked/execd so check for wake up only.
	 */
	if (enable_preemption && (wake_flags & SCX_WAKE_TTWU)) {
		...
		bpf_for(i, 0, domcpus->nr_cpus) {
			...
			if (!pcpuc) {
				scx_bpf_error("No pcpu_ctx");
				goto enoent;
			}

			if (bpf_cpumask_test_cpu(cpu_i, p->cpus_ptr)
			&& !bpf_cpumask_test_cpu(cpu_i, pre_cpumask)
			&& vtime_before(p->scx.dsq_vtime, pcpuc->task_vtime)) {
				cpu = cpu_i;
				bpf_cpumask_set_cpu(cpu, pre_cpumask);
